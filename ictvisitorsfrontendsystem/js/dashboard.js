...
async function loadVisitors() {
  try {
    const res = await fetch("https://localhost:7099/api/visitors");
...
async function deleteVisitor(id) {
  ...
  try {
    const original = await fetch(`https://localhost:7099/api/visitors/${id}`);
    const visitor = await original.json();
    const res = await fetch(`https://localhost:7099/api/visitors/${id}`, { method: "DELETE" });
    if (!res.ok) throw new Error("Failed to delete visitor");
    lastDeletedVisitor = visitor;
    showToastWithUndo("🗑️ Visitor deleted.", async () => {
      const undoRes = await fetch("https://localhost:7099/api/visitors", {
        method: "POST",
...
document.getElementById("visitorForm").addEventListener("submit", async function (e) {
  ...
  const url = visitor.id
    ? `https://localhost:7099/api/visitors/${visitor.id}`
    : "https://localhost:7099/api/visitors";
  try {
